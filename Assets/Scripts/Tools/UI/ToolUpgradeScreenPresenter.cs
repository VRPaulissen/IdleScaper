...
namespace Tools.UI
{
    ...
    /// </summary>
    public sealed class ToolUpgradeScreenPresenter : MonoBehaviour
    {
        [Header("View")]
        [SerializeField] private ToolUpgradeScreenView view;
        ...
        {
            if (view == null)
                view = GetComponent<ToolUpgradeScreenView>();
        ...
        private bool TryInitialize()
        {
            if (view == null)
            {
                Logger.LogError($"{nameof(ToolUpgradeScreenPresenter)} missing view.");
                return false;
            }

            if (saveBootstrap == null)
            {
                Logger.LogError($"{nameof(ToolUpgradeScreenPresenter)} missing SaveBootstrap.");
                return false;
            }

            if (itemDatabase == null)
            {
                Logger.LogError($"{nameof(ToolUpgradeScreenPresenter)} missing ItemDatabase.");
                return false;
            }

            if (toolDefinitionCatalog == null)
            {
                Logger.LogError($"{nameof(ToolUpgradeScreenPresenter)} missing ToolDefinitionCatalog.");
                return false;
            }

            if (recipeCatalog == null)
            {
                Logger.LogError($"{nameof(ToolUpgradeScreenPresenter)} missing ToolUpgradeRecipeCatalog.");
                return false;
            }
            ...
            if (saveData == null)
            {
                Logger.LogError($"{nameof(ToolUpgradeScreenPresenter)} could not resolve save data.");
                return false;
            }
